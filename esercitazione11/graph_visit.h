...
lista_adiacenza(unidirected_graph graph){
    if (graph.all_nodes().empty()) return {};
    ...
    std::vector<std::list<int>> array(max_node + 1);
    
    for (auto edge : graph.all_edges()){
        if (edge.from() != edge.to()) { //no self-loop
            array[edge.from()].push_back(edge.to());
            array[edge.to()].push_back(edge.from());
        }
    }
    return array;
}
